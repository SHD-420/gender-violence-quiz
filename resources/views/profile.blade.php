<x-dashboard-layout>
    ...
    <form class="w-1/2 mx-auto p-8 shadow-xl bg-white rounded-md" method="POST" action="{{ route('profile') }}">
        @csrf
        <h4 class="text-2xl text-gray-600 font-bold mb-4">Edite su perfil</h4>
        <div class="mb-2">
        ...
        <div class="flex space-x-8 mb-8">
            ...
            <div class="flex space-x-4 items-center">
                ...
                <input type="number" value="{{ auth()->user()->age }}"
                    class="w-36 rounded-md py-2 px-4 bg-gray-100 border-2  focus:ring-2 outline-none" name="age"
                    id="" placeholder="Eg: 24">
            </div>
        </div>
        <button type="submit"
            class="text-center w-full bg-purple-800 text-white font-semibold py-2 rounded-md hover:bg-purple-700">Salvar</button>
    </form>
</x-dashboard-layout>
